...
/**
 * Component này dùng để nhập ngày tháng có thể tương thích với CalendarChooser
 * 
 * @author Phuc Hung
 * 
 */
...
public class DateTextField extends JPanel {
	...
	private MyTextField txtDate = null;
	...
	public DateTextField(float fontSize) {
	...
	public void setLimitAge(final int age, String mess) {
		...
		if (checkAgeDListener != null) {
			txtDate.getDocument().removeDocumentListener(checkAgeDListener);
		...
		txtDate.getDocument().addDocumentListener(getAgeListener(age));
		...
		if (date != null) {
			if (!date.equals(getFullDate())) {
				if (date.length() == 4) {
					...
					if (date.length() == 10) {
						txtDate.setText(date.substring(0, 2));
	...
	public void setDate(String date) {
		if (date != null) {
			...
			System.out.println(!date.equals(getFullDate()));
			if (!date.equals(getFullDate())) {
				if (date.length() == 4) {
					txtDate.setText("01");
					txtMonth.setText("01");
	...
	public Timestamp getTimeStamp() {
		...
		String month = txtMonth.getText().equalsIgnoreCase("") ? null
				: txtMonth.getText();
		@SuppressWarnings("unused")
		String date = txtDate.getText().equalsIgnoreCase("") ? null : txtDate
				.getText();
		if (year == null)
			return null;
	...
	public Calendar getCalendar() {
		...
		int date = Integer.valueOf(txtDate.getText());
		cal.clear();
	...
	public boolean isValidDate() {
		String strDate = "";
		strDate = txtDate.getText() + "-" + txtMonth.getText() + "-"
				+ txtYear.getText();
	 ...
	 */
	private MyTextField getTxtDate() {
		if (txtDate == null) {
			txtDate = new MyTextField();
			txtDate.setHorizontalAlignment(JTextField.RIGHT);
			txtDate.setDocument(new RangeNumber(1, 31));
			txtDate.setBorder(null);
			txtDate.setText("");
			txtDate.addKeyListener(new KeyAdapter() {
				public void keyReleased(KeyEvent e) {
					if (e.getKeyChar() >= '0' && e.getKeyChar() <= '9')
						if (txtDate.getText().length() == 2) {
							txtDate.transferFocus();
						}
			...
			txtDate.addFocusListener(new FocusAdapter() {
				...
				public void focusGained(FocusEvent e) {
					...
					txtDate.setSelectionEnd(txtDate.getText().length());
				}

				public void focusLost(FocusEvent e) {
					if (txtDate.getText().length() == 0) {
						txtDate.setText("01" + txtDate.getText());
					} else if (txtDate.getText().length() == 1) {
						if ("0".equalsIgnoreCase(txtDate.getText())) {
							txtDate.setText("01");
						} else {
							txtDate.setText("0" + txtDate.getText());
						}
					}
					if (listener != null) {
						if (!txtMonth.isFocusOwner() && !txtYear.isFocusOwner())
							listener.actionPerformed(new ActionEvent("", 0, ""));
					}
				}
			});
		...
		if (txtMonth == null) {
			...
			txtMonth.addFocusListener(new FocusAdapter() {
				...
				public void focusLost(FocusEvent e) {
					...
					if (listener != null)
						if (!txtDate.isFocusOwner() && !txtYear.isFocusOwner())
							listener.actionPerformed(new ActionEvent("", 0, ""));
				}
			});
		}
	...
	public void addKeyListener(KeyListener keyListener) {
		txtDate.addKeyListener(keyListener);
		...
		if (txtYear == null) {
			...
			txtYear.addFocusListener(new FocusAdapter() {
				...
				public void focusLost(FocusEvent arg0) {
					...
					if (txtDate.getText().length() == 0) {
						txtDate.setText("01" + txtDate.getText());
					} else if ("0".equalsIgnoreCase(txtDate.getText())) {
						txtDate.setText("01");
					}
					...
					if (listener != null)
						if (!txtMonth.isFocusOwner() && !txtDate.isFocusOwner())
							listener.actionPerformed(new ActionEvent("", 0, ""));
				}
			});
		}
