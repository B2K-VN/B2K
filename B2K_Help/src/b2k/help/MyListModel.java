...
/**
 * This class loosely implements the <code>java.util.Vector</code> API, in
 * that it implements the 1.1.x version of <code>java.util.Vector</code>, has
 * no collection class support, and notifies the <code>ListDataListener</code>s
 ...
 * @author Hans Muller
 */
public class MyListModel extends AbstractListModel implements MutableComboBoxModel, Serializable {
	/**
	 * 
	 */
	...
	public MyListModel(final Object items[]) {
		if(items != null){
	  	delegate.ensureCapacity( items.length );
		int i,c;		
			for ( i=0,c=items.length;i<c;i++ )
				delegate.addElement(items[i]);		
			if ( getSize() > 0 ) {
	...
	@SuppressWarnings("unchecked")
	public MyListModel(List<?> v) {
		if (v != null)
		for (int i = 0; i < v.size(); i++) {
			delegate.addElement(v.get(i));
		}		
	}
	...
	public void addAll(Collection c) {
		...
		while(iterator.hasNext()){
			delegate.addElement(iterator.next());
		}
		int index1 = getSize()-1;
		fireIntervalAdded(delegate.get(index1), index1, index1);
	}

	@SuppressWarnings("rawtypes")
	public boolean removeAll(Collection c){
		boolean modified = false;
		Iterator<?> e = delegate.iterator();
		while (e.hasNext()) {
	...
	/**
	 * Chớ nên dùng cái này.. Lỗi ráng chịu
	 * @return
	 */
	public Collection<?> getValues(){
		int l = delegate.size();
		try{
			return delegate.subList(0, l<0?0:l);
		}catch(Exception e){
			l = delegate.size() - 1;
			return delegate.subList(0, l<0?0:l);
		}
